		...
		for (i = 0; i < 100000; i++)
		{
			...
			if (account_temp == account)
			{
				if (pass_temp == pass)
				{
					for (int i = 0;; i++)
					{
						fscanf(tripFile, "%d\t%d\t%d\t%d\t%d:%d\t%d:%d\t%d/%d/%d\t%s\t%d\n", &driverUsername, &origin_temp, &destination_temp, &way_temp, &startTimeHour, &startTimeMinute, &timeDurationHour, &timeDurationMinute, year_ttrip, month_tttip, day_ttrip, vehicle_temp, &cast_temp);
